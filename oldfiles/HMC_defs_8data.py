...
def calculate_gammaL(P0_total, Tcenter, T_seal, VMR, predictions, mdb_voigt, linenum):
    P_total = P0_total * Tcenter / T_seal
    P_self = P_total * VMR

    ns = jnp.array([predictions[f"n{i}"] for i in range(1, linenum + 1)])

    # Check if gamma_broads should be vectorized
    if any(f"gamma_broad{i}" in predictions for i in range(1, linenum + 1)):
        gamma_broads = jnp.array(
            [predictions[f"gamma_broad{i}"] for i in range(1, linenum + 1)]
        )
    else:
        gamma_broads = mdb_voigt.gamma_air
    ...
    # Handle the case where gamma_broads and gamma_selfs are arrays
    if gamma_broads.shape == () and gamma_selfs.shape == ():  # Scalars
        gammaL = vmap(gamma_hitran, in_axes=(None, None, None, 0, None, None))(
            P_total, Tcenter, P_self, ns, gamma_broads, gamma_selfs
        )
    else:  # Arrays
        gammaL = jnp.array(
            [
                gamma_hitran(
                    P_total, Tcenter, P_self, ns[i], gamma_broads[i], gamma_selfs[i]
                )
...
def calculate_gammaL_peratm(Tcenter, VMR, predictions, mdb_voigt, linenum):
    ...
    if gamma_broads.shape == () and gamma_selfs.shape == ():  # Scalars
        gammaL = vmap(gamma_hitran, in_axes=(None, None, None, 0, None, None))(
            P_1atm, Tcenter, P_self_1atm, ns, gamma_broads, gamma_selfs
        )
    else:  # Arrays
        gammaL_peratm = jnp.array(
            [
                gamma_hitran(
                    P_1atm, Tcenter, P_self_1atm, ns[i], gamma_broads[i], gamma_selfs[i]
                )
                for i in range(linenum)
            ]
        )
...
def calc_keyarray_2cell(
    ...
    gammaLkey = [
        f"{prefix}_{Tcenters[i]}_{j+1}"
        for prefix in ["gammaL", "gself_Tatm", "gbroad_Tatm"]
        for i in range(nTcenter)
        for j in range(linenum)
    ]
    gammaLadd_keyarray = keyarray + gammaLkey
    ...
    for key in YgammaLadd_keyarray:
        if key in gammaLkey:
            for i in range(nTcenter):
                ...
                gbroad_Tatm_array = calculate_gammaL_peratm(
                    Tcenters[i],
                    0.0,
                    predictions,
                    mdb_voigt,
                ...
                for j in range(linenum):
                    ...
                    key = f"gbroad_Tatm_{Tcenters[i]}_{j+1}"
                    hpdi_values[key] = hpdi(gbroad_Tatm_array[j], 0.9)  # 90% range
                    median_value[key] = np.median(gbroad_Tatm_array[j], axis=0)
