...
start_wavelength = 1628.18
end_wavelength = 1628.335
...
linenum = 3  # number of lines for Voigt fitting
...
"""
# Read the quantum numbers from HITEMP
mdb_nonact = MdbHitemp(
    "CH4",
    nurange=nu_grid,
    activation=False,
    parfile = "database/06_HITEMP2020.par",
)  # for obtaining the error of each line

# Filter dataframe based on the nu_grid nurange(line numbers of mdb)
df_filter_wav = mdb_nonact.df[
    (mdb_nonact.df["wav"] >= nu_min - adjustrange)
    & (mdb_nonact.df["wav"] <= nu_max + adjustrange)
]
# print("Filtered Line amount: ", len(df_filter_wav.wav.values))

# Convert Jlower codes from mdb(non-activated)
Jlower_str = [str(value) for value in df_filter_wav.locl.values]

# Fixed: Check string length and ensure it's a digit before converting to int
Jlower = []
symmetry = []
for value in Jlower_str:
    if len(value) >= 4:  # Ensure at least 4 characters to avoid index out of range
        jlower_str = value[3:5].strip()  # Extract the part, remove spaces
        if jlower_str.isdigit():  # Check if it's a valid number
            Jlower.append(
                int(jlower_str.zfill(2))
            )  # Always treat it as a two-digit number
            symmetry.append(value[5:7])  # Extract symmetry
        else:
            Jlower.append(-1)  # Assign -1 for invalid values
            symmetry.append("XX")  # Assign 'XX' for invalid symmetry values
            # print(f"Invalid value detected, set to -1: '{value}'")
    else:
        Jlower.append(-1)  # Assign -1 for too short values
        symmetry.append("XX")  # Assign 'XX' for invalid symmetry values
        # print(f"Too short value detected, set to -1: '{value}'")
"""
...
""" half way of creating
for i in range(len(nu_center_voigt)):
    print(
        mdb_voigt.nu_lines_err[strline_ind_array_nu[i]],
        mdb_voigt.line_strength_ref_err[strline_ind_array_nu[i]],
        mdb_voigt.gamma_air_err[strline_ind_array_nu[i]],
        mdb_voigt.gamma_self_err[strline_ind_array_nu[i]],
        mdb_voigt.n_air_err[strline_ind_array_nu[i]],
    )
"""
"""
# Find the indices of the closest matching wavenumbers in mdb_act.nu_lines
closest_indices = find_closest_indices(df_filter_wav.wav.values, nu_center_voigt)

# Output the corresponding Jlower value
corresponding_Jlowers = [Jlower[idx] for idx in closest_indices]
"""
...
def model_c(y1, y2, y3, y4, y5, y6, y7, y8):
    ...
    """
    # Create a single set of samples for each ln, all with the name "alpha1_{ln+1}"
    alpha_samples = [
        numpyro.sample(f"alpha1_{ln + 1}", dist.Uniform(0.0, 5.0))
        for ln in range(linenum)
    ]

    # Construct the alphas array, reusing the same sample set for all nspec values
    alphas = jnp.array(
        [
            alpha_samples[ln]  # Use the same sample for all nspec values
            for ns in range(nspec)
            for ln in range(linenum)
        ]
    ).reshape(nspec, linenum)
    ...
    gamma_H2Hes = jnp.array(
        [
            numpyro.sample(f"gamma_H2He{i}", dist.Uniform(0.0, 0.2))
            # numpyro.sample(f"gamma_self{i}", dist.Uniform(0.060, 0.062))
            for i in range(1, 1 + linenum)
        ]
    )
    ...
    n_H2Hes = jnp.array(
        [
            numpyro.sample(f"n_H2He{i}", dist.Uniform(-2.0, 2.0))
            for i in range(1, 1 + linenum)
        ]
    )
    n_selfs = jnp.array(
        [
            numpyro.sample(f"n_self{i}", dist.Uniform(-2.0, 2.0))
            for i in range(1, 1 + linenum)
        ]
    )
        ...
        coeffs.append(
            {
                "a": numpyro.sample(
                    ...
                    dist.Uniform(-(valrange**3), valrange**3),
                ),
                "b": numpyro.sample(
