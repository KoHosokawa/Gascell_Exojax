...
start_wavelength = 1621.8
end_wavelength = 1621.89
...
def model_c(y1, y2, y3, y4, y5, y6, y7, y8):
    ...
    n_H2Hes = jnp.array(
        [
            numpyro.sample(f"n_H2He{i}", dist.Uniform(0.0, 2.0))
            for i in range(1, 1 + linenum)
        ]
    )
    n_selfs = jnp.array(
        [
            numpyro.sample(f"n_self{i}", dist.Uniform(0.0, 2.0))
            for i in range(1, 1 + linenum)
        ]
    )
...
# Save file name
savefilename = f"Results/HMC/Multifit/240701-0703_{wavmin_str}-{wavmax_str}_R00025_CH4VMR01-1_HMC{Nitr}_{linenum}V_sig1e+3_expdist_nuoff-005_n-both0-2_g-both0-02_pval3_al0-3_unidist_{nspec}dfit"
